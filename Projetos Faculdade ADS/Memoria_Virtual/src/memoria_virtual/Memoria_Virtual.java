...
public class Memoria_Virtual {
    ...
    public static void main(String[] args) {
       /*
		 ...
		 A memoria virtual � dividida em 5 paginas*
		 Pode haver at� 5 processos na memoria virtual
		 */
		...
		for(int i = 0; i < numMinCiclos; i++) {
			...
			if(!memoriaFisica.isEmpty()) {
				...
				if(memoriaVirtual.contains(memoriaFisica.get(0))) {// apenas vai executando o processo
					...
					//setar atributos para chamada da fun��o posteriormente
					...
					if(memoriaVirtual.size() < numPaginas) {//h� espa�o na memoria virtual, carregar processo para a pagina
						int numDaPage = memoriaVirtual.size() + 1;
						System.out.println("H� espa�o na memoria virtual, processo "+  memoriaFisica.get(0).getId() + " carregado para a pagina " + numDaPage);//verificar em qual pagina o processo sera carregado
						// setar atributos para chamda da fun��o posteriormente
						memoriaVirtual.add(memoriaFisica.get(0));
						
						
					}else {//n�o h� mais espa�o na memoria virtual, subtituir pagina mais antiga, escalonamento circular!!!
						memoriaVirtual.remove(0);
						memoriaVirtual.add(memoriaFisica.get(0));
						System.out.println("N�o h� mais espa�o na memoria virtual, acionando bluffler circular e sobreescrevendo pagina mais antiga");
						//setar atributos para chamada da fun��o posteiriomente
						
					}
				}
				if(Gerenciador.processar(memoriaFisica.get(0))) {// se o processo foi finalizado
					//liberar processo da memoria fisica e virtual
					System.out.println("Processo " + memoriaFisica.get(0).getId() + " Finalizado, liberar espa�o na memoria virtual de na memoria fisica...");
		...
		while(!memoriaFisica.isEmpty()) {
			...
			if(Gerenciador.processar(memoriaFisica.get(0))) {// se o processo foi finalizado
				//liberar processo da memoria fisica e virtual
				System.out.println("Processo " + memoriaFisica.get(0).getId() + " Finalizado, liberar espa�o na memoria virtual de na memoria fisica... (removida pagina em que se encontrava o processo)");
				memoriaVirtual.remove(memoriaFisica.get(0));
				memoriaFisica.remove(0);
			}else {
				if(memoriaFisica.get(0).getControleQuantum() < quantum) {
...
//classe para gerenciar o processo em execu��o
class Gerenciador{
